...
- [x] トップページ (/) のランディングページ (LP) 化
- [x] 外部公開向け LP のブラッシュアップ（管理用導線削除、日本語化、メタデータ設定）
- [x] 研修・営業提案用デモシナリオ (docs/demo-scenario.md) の作成
- [x] **OpenAI API によるパーソナライズアドバイス生成機能の実装**
- [x] AI 生成結果の Supabase 保存および共有表示対応
- [x] **レポート生成中のローディング UX 改善（全画面オーバーレイ、ステップ表示）**

## 動作確認済みの内容（Vercel公開環境）
- [x] **/ (トップページ)**: LP 化およびメタデータ設定を確認。
...
- [x] **ドキュメント同期**: `npm run sync:obsidian` による自動同期機能の動作を確認。
